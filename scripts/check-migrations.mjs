...
export const REQUIRED_HISTORICAL_REPAIR_FILES = [
  '20260419180000_restore_notes_startup_snapshot.sql',
  '20260419203000_restore_org_wiki_startup_snapshot.sql',
  '20260419204000_repair_ai_personas_model_drift.sql',
];
...
export function validateRequiredHistoricalRepairs(
  entries,
  {
    activeMigrationsRelative = ACTIVE_MIGRATIONS_RELATIVE,
    requiredHistoricalRepairFiles = REQUIRED_HISTORICAL_REPAIR_FILES,
  } = {},
) {
  const missingRepairFiles = requiredHistoricalRepairFiles.filter((entry) => !entries.includes(entry));

  return {
    errors: missingRepairFiles.length === 0
      ? []
      : [`Missing required historical repair migrations in ${activeMigrationsRelative}: ${missingRepairFiles.join(', ')}`],
    missingRepairFiles,
    ok: missingRepairFiles.length === 0,
  };
}
...
export function runMigrationGuard({
  ...
  const requiredRepairValidation = validateRequiredHistoricalRepairs(activeEntries, { activeMigrationsRelative });
  let archiveEntries = [];
  ...
  errors.push(...requiredRepairValidation.errors);
