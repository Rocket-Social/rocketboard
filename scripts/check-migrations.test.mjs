...
function createRepoFixture({
  activeEntries = FROZEN_BASELINE_FILES,
...
describe('check-migrations', () => {
  it('passes for frozen baseline files, a valid forward migration, and a normalized archive', () => {
    const fixture = createRepoFixture({
      activeEntries: [...FROZEN_BASELINE_FILES, '20260412123045_add_workspace_slug.sql'],
      archiveEntries: [
  ...
  it('fails on duplicate migration slugs across active and archived files', () => {
    const fixture = createRepoFixture({
      activeEntries: [...FROZEN_BASELINE_FILES, '20260412123045_ai_subscription_auth.sql'],
    ...
    expect(result.ok).toBe(false);
    expect(result.errors).toContain(
      'Duplicate migration slug ai_subscription_auth: supabase/migrations/20260412123045_ai_subscription_auth.sql, supabase/migrations_archive/2026-reset/legacy_v15a_ai_subscription_auth.sql',
    );
  });
  ...
  it('allows append-only additions relative to a base ref', () => {
    ...
    writeFixtureFile(
      path.join(fixture.repoRoot, ACTIVE_MIGRATIONS_RELATIVE, '20260412123045_add_workspace_slug.sql'),
    );
  ...
  it('fails when an added migration backdates itself before the current active head', () => {
    const fixture = createRepoFixture({
      activeEntries: [...FROZEN_BASELINE_FILES, '20260412123045_latest_change.sql'],
    });
    initGitRepo(fixture.repoRoot);
    ...
    expect(result.ok).toBe(false);
    expect(result.errors).toContain(
      'New active migration must move history forward; 20260411120000_backdated_change.sql is not newer than existing version 20260412123045.',
    );
  });
