...
begin
  ...
  return query
    ...
    -- profiles
    --   -> search match + non-guest org tier filter
    --   -> page users
    --   -> hydrate ordered non-guest memberships for just that page
    paged_users as (
      select
        p.user_id,
        p.email,
    ...
    membership_rows as (
      ...
      join public.organization_members om
        on om.user_id = paged_users.user_id
       and om.role <> 'guest'
      join public.organizations o on o.id = om.organization_id
      join lateral public.get_org_effective_entitlements(o.id) ent on true
    )
    select
      ...
      (
        select count(*)
        from public.organization_members om
        where om.user_id = paged_users.user_id
          and om.role <> 'guest'
      ),
