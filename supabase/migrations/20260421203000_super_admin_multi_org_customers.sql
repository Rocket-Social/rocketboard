...
begin
  ...
  return query
    ...
    paged_users as (
      ...
      cross join clock
      where not coalesce(p.is_internal_admin, false)
      and (
        p_search is null
...
revoke all on function public.super_admin_get_customers(text, text, int, int) from public;
grant execute on function public.super_admin_get_customers(text, text, int, int) to authenticated;
