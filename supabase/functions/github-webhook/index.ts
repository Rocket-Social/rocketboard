...
async function handlePullRequestReview(supabase: ReturnType<typeof createServiceClient>, payload: Record<string, unknown>) {
  ...
  const installation = payload.installation as Record<string, unknown> | undefined
  const token = await resolveWebhookToken(repoRecords, installation)
  const reviews = token ? await fetchPullRequestReviews(String(repo.full_name ?? ''), Number(pr.number), token) : [review]
  const requestedReviewers = Array.isArray(pr.requested_reviewers) ? pr.requested_reviewers as Record<string, unknown>[] : []
  const {
    approvalCount,
