...
describe('buildEdgeExceptionEvent', () => {
  ...
  it('normalizes bare async stack lines to keep PostHog frame functions present', () => {
    const error = new Error('boom')
    error.stack = [
      'Error: boom',
      '    at async file:///srv/functions/example.ts:68:21',
      '    at ext:runtime/http.js:246:20',
    ].join('\n')
