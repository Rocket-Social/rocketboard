...
export function errorResponse(message: string, status = 400, code?: string) {
  const payload = code ? {code, error: message} : {error: message}
...
export async function errorResponseForException(
  err: unknown,
  ...
  // 5xx path — wait for the monitoring request before returning so handled
  // exceptions don't get dropped when the runtime tears down the response.
  if (functionName) {
    await captureEdgeException(err, {functionName})
  }
...
export async function requireInternalAdmin(req: Request) {
  const user = await getAuthenticatedUser(req)
  if (!user) throw new HttpError('Unauthorized', 401)
  ...
  if (!profile?.is_internal_admin) {
    throw new HttpError('Internal admin access required', 403)
  }
