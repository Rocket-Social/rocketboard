...
function parseStackFrame(line: string): EdgeExceptionFrame | null {
  ...
  const namedFrameMatch = /^(?:at\s+)?(.*?)\s+\((.+):(\d+):(\d+)\)$/.exec(trimmed)
  if (namedFrameMatch) {
    const [, functionName, filename, lineNumber, columnNumber] = namedFrameMatch
    return {
      platform: 'custom',
      lang: 'javascript',
  ...
  const locationOnlyMatch = /^(?:at\s+)?(.+):(\d+):(\d+)$/.exec(trimmed)
  if (!locationOnlyMatch) return null

  const [, rawFilename, lineNumber, columnNumber] = locationOnlyMatch
  const filename = rawFilename.replace(/^async\s+/, '')
  return {
    platform: 'custom',
    lang: 'javascript',
    function: '<anonymous>',
