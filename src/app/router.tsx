...
import {
  inboxRoutePath,
  validateInboxSearch,
} from "../features/inbox/inbox.routes";
import {
  myNotesRoutePath,
...
import { ENABLE_E2E_HARNESS } from "./config";
...
const InboxPage = lazyWithRetry(() =>
  import("../features/inbox/InboxPage").then((module) => ({
...
const VipBillingHarnessPage = ENABLE_E2E_HARNESS
  ? lazyWithRetry(() =>
    import("../features/org-settings/VipBillingHarnessPage").then((module) => ({
      default: module.VipBillingHarnessPage,
    })),
  )
  : null;
const CanvasCompactHeightHarnessPage = ENABLE_E2E_HARNESS
  ? lazyWithRetry(() =>
    import("../features/canvas/CanvasCompactHeightHarnessPage").then((module) => ({
      default: module.CanvasCompactHeightHarnessPage,
    })),
  )
  : null;
...
const inboxRoute = createRoute({
  loader: async ({ context, location }) => {
    await loadSignedInAppFrameData(context, location);
    await preloadLazyComponent(InboxPage);
  },
  component: InboxPage,
  getParentRoute: () => utilityShellLayoutRoute,
  path: inboxRoutePath,
...
const vipBillingHarnessRoute = ENABLE_E2E_HARNESS && VipBillingHarnessPage
  ? createRoute({
    component: VipBillingHarnessPage,
    getParentRoute: () => rootRoute,
    path: "/__e2e/vip-billing",
  })
  : null;
const canvasCompactHeightHarnessRoute = ENABLE_E2E_HARNESS && CanvasCompactHeightHarnessPage
  ? createRoute({
    component: CanvasCompactHeightHarnessPage,
    getParentRoute: () => rootRoute,
    path: "/__e2e/canvas-compact-height",
  })
  : null;
...
const routeTree = rootRoute.addChildren([
  ...
  authCallbackRoute,
  devThrowRoute,
  ...(vipBillingHarnessRoute ? [vipBillingHarnessRoute] : []),
  ...(canvasCompactHeightHarnessRoute ? [canvasCompactHeightHarnessRoute] : []),
  onboardingRoute,
  resetPasswordRoute,
  acceptInviteRoute,
