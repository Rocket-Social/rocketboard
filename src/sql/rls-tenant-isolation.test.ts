...
describe('RLS tenant-isolation regression guards', () => {
  ...
  // Phase 7-A (D7-4): tightened further to admin-only — non-admin members
  // can no longer infer whether an org has API keys configured.
  itIfPg('ai_api_keys SELECT restricts org-scoped rows to admins (D7-4)', () => {
    const qual = psqlQuery(
      `select qual
       from pg_catalog.pg_policies
    ...
    expect(qual).not.toContain("'member'")
  })
