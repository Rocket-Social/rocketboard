...
type DbOrgBillingAdminRow = {
  adminGrantStartsAt: string | null
  hasBillingCustomer: boolean
  vipCanceledSubscriptionId: string | null
  vipCancellationManaged: boolean | null
}
...
export const entitlementRepository = {
  ...
  async getOrgEntitlements(
    ...
    return {
      ...
      adminGrantStartsAt: adminSnapshot?.adminGrantStartsAt
        ? new Date(adminSnapshot.adminGrantStartsAt).getTime()
        : null,
      adminGrantEndsAt: summary.adminGrantEndsAt ? new Date(summary.adminGrantEndsAt).getTime() : null,
      vipCancellationManaged: Boolean(adminSnapshot?.vipCancellationManaged),
      vipCanceledSubscriptionId: adminSnapshot?.vipCanceledSubscriptionId ?? null,
