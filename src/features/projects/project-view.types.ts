...
export function resolveProjectOverviewConfig(
  ...
  const overviewSprintIds = normalizeProjectConfigIdList(
    state?.overviewSprintIds
    ?? (typeof state?.overviewSprintId === 'string' ? [state.overviewSprintId] : []),
  )
  ...
  return {
    ...
    overviewSprintIds,
    overviewSprintId: overviewSprintIds[0] ?? null,
