...
export type BoardViewGroupBy = 'status' | 'assignee'
...
type BoardViewProps = {
  // Phase 4 PR 4-B-2: persona list flows through to BoardTaskCard so
  // <AssigneeIdentity> can render agents with their persona avatar +
  // accent color instead of falling back to "Unknown".
  ...
  // Phase 4 PR 4-B-2: 'status' (default — current behavior) or
  // 'assignee' (drop = setCardAssignee → trigger dispatches agent).
  groupBy?: BoardViewGroupBy
  ...
  // Phase 4 PR 4-B-2: assignee-mode drag handler. Required when
  // groupBy='assignee'; ignored otherwise.
  onMoveAssignee?: (input: {
    cardId: string
...
function columnAccent(
  ...
  if (groupBy === 'assignee') {
    return column.accentColor ?? 'var(--color-border-subtle)'
  }
  const option = statusOptions.find((o) => o.id === column.id)
      ...
      <div className='flex items-center justify-between gap-3'>
        <div className='flex min-w-0 items-center gap-2'>
          <AssigneeIdentity
            ...
            size='sm'
            userId={task.card.assigneeUserId}
          />
...
function BoardColumnBody({
  ...
  // In assignee mode the +Add task button skips the column-derived
  // default and opens the composer fresh — CreateCardInput doesn't
  // carry assigneeUserId today, so the user picks the assignee inside
  // the modal. Status mode keeps its statusOptionId preset.
  const createTaskDefaultsForColumn = groupBy === 'assignee'
    ? undefined
    : createTaskDefaults(column.id, 'standard')

  return (
    <BoardColumnLane droppableId={`lane:${column.id}:${taskBoardStandardLaneId}`} scrollRef={scrollRef}>
      ...
      <button
        ...
        onClick={() => onCreateTask(createTaskDefaultsForColumn)}
        type='button'
      >
...
function CollapsedBoardColumn({
  ...
  const droppableEnabled = groupBy === 'assignee' || taskMode === 'standard'
  const droppableId = droppableEnabled ? `lane:${column.id}:${taskBoardStandardLaneId}` : `collapsed:${column.id}`
      ...
      <div className='flex flex-1 flex-col items-center gap-3 px-2 py-3'>
        ...
        <div className='flex w-full flex-col gap-1.5 border-t border-border-subtle pt-2'>
          ...
          <button
            ...
            className={boardColumnActionButtonClassName}
            onClick={() =>
              onCreateTask(groupBy === 'assignee' ? undefined : createTaskDefaults(column.id, taskMode))
            }
...
export function BoardView({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (groupBy === 'assignee') {
      // Phase 4 PR 4-B-2: drag-as-dispatch. The Phase 2a trigger
      // (cards_assignee_dispatch) inserts an ai_agent_runs row when
      // the new assignee resolves to a persona's agent_user_id; for
      // a human column the trigger no-ops (REG-2). Position bookkeeping
      // doesn't apply — assignee mode doesn't reorder by status.
      const targetAssigneeUserId = targetColumnId === ASSIGNEE_UNASSIGNED_COLUMN_ID
        ? null
        : targetColumnId
      const previousAssigneeUserId = source.task.card.assigneeUserId
      if (previousAssigneeUserId === targetAssigneeUserId) {
        return
      }

      setOptimisticBoardTasks(nextBoardTasks)

      const succeeded = await (onMoveAssignee?.({
    ...
    >
      <div className='flex gap-4 overflow-x-auto pb-4'>
        {boardColumns.map((column) => {
          ...
          const accentColor = columnAccent(mode, column, statusOptions, groupBy)
          const headerCreateDefaults = groupBy === 'assignee'
            ? undefined
            : createTaskDefaults(columnId, taskMode)
            ...
            >
              {collapsed ? (
                ...
                <>
                    ...
                    <div className='flex items-center justify-between border-b border-border-subtle px-3 py-3'>
                      ...
                      <div className='flex items-center gap-1 text-text-muted'>
                        ...
                        <button
                          aria-label={`Add task to ${column.title}`}
                          className={boardColumnActionButtonClassName}
                          onClick={() => onCreateTask(headerCreateDefaults)}
                          title={`Add task to ${column.title}`}
                          type='button'
                        >
                          <Plus className='h-3.5 w-3.5' />
                  ...
                  {taskMode === 'sprint' && groupBy === 'status' ? (
        ...
        <button
          ...
          onClick={() => onCreateTask(taskMode === 'sprint' && groupBy === 'status' ? {sprintId: null} : undefined)}
          type='button'
        >
          <Plus className='h-4 w-4' />
        </button>
