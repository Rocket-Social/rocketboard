...
describe('TableView inline task creation', () => {
  ...
  it('hides the group field even if it was previously visible in sprint mode', () => {
    const sprint = createSprintRecord('sprint-1', 'Sprint 1')

    renderTableView({
      expandedGroups: ['sprint-1'],
      groupBy: 'status',
      projectGroups: [createGroupRecord('group-1', 'Alpha')],
      taskMode: 'sprint',
      tableGroups: [
        createGroup('sprint-1', 'Sprint 1', [
          createTask('task-1', 'Sprint task', {groupId: 'group-1', sprintId: sprint.id}),
...
describe('TableView custom text fields', () => {
  it('renders long text values in a truncating container', () => {
    const longValue = 'https://docs.google.com/document/d/very-long-value-that-should-not-overflow-the-column-boundary'
    const customFields: CustomFieldDefinition[] = [
      {
        fieldType: 'text',
        id: 'field-1',
        key: 'custom:text:links',
        name: 'Links',
        options: [],
      },
    ]
    ...
    renderTableView({
      ...
      visibleFieldKeys: ['custom:text:links'],
    })

    const textCell = screen.getByText(longValue)
    expect(textCell).toHaveClass('block', 'w-full', 'max-w-full', 'truncate', 'text-left')
    expect(textCell).toHaveAttribute('title', longValue)
  })
})
