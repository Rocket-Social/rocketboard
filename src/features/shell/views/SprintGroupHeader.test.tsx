...
describe('SprintGroupHeader', () => {
  ...
  it('offers Mark complete on planned sprints — covers lapsed/past-dated sprints whose owner forgot to start them', async () => {
    const user = userEvent.setup()
    ...
    render(
      <SprintGroupHeader
        ...
        onStartSprint={vi.fn()}
        onToggle={vi.fn()}
        sprint={makeSprint({status: 'planned'})}
        statusOptions={[]}
        taskCount={0}
        tasks={[]}
      />,
  ...
  it('does not offer Mark complete on active sprints (the primary Complete Sprint button already handles them)', async () => {
    const user = userEvent.setup()
    ...
    render(
      <SprintGroupHeader
        ...
        onStartSprint={vi.fn()}
        onToggle={vi.fn()}
        sprint={makeSprint({status: 'active'})}
        statusOptions={[]}
        taskCount={0}
        tasks={[]}
      />,
