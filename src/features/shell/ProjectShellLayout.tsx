...
export function ProjectShellLayout() {
  ...
  const shouldShowToolbarArea = activeViewTypeSegment !== 'canvas'
  ...
  const header = (
    <>
      ...
      {shouldShowToolbarArea ? <ToolbarAreaSlot /> : null}
    </>
  )
  ...
  const dialogs = (
    <>
      <WorkspaceCommandPalette
        ...
        onOpenMyNote={async (hit) => {
          if (!(await beforeSidebarNavigate())) {
            return false
          }
          void navigateWhenWarm({
            location: {
              search: buildMyNotesSearch(currentWorkspace.slug, hit.noteId),
              to: myNotesRoutePath,
            },
            navigate,
            router,
