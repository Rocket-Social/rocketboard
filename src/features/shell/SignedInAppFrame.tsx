...
import { useUserActiveHeartbeat } from "../auth/heartbeat";
import { useInboxRealtime } from "../inbox/inbox.realtime";
...
export function SignedInAppFrame() {
  ...
  useUserActiveHeartbeat(session?.status === "authenticated");
  // Mount the inbox realtime channel globally so the sidebar unread badge
  // updates whether or not the user is on `/inbox`.
  useInboxRealtime(session?.status === "authenticated" ? session.user.id : null);
