...
describe('useInitializeViewSearch', () => {
  ...
  it('merges shared-only fields after a personal bootstrap', async () => {
    ...
    const {rerender} = renderHook(({isBaselineReady, search}: {isBaselineReady: boolean; search: SearchParams}) => useInitializeViewSearch<SearchParams, SharedBaseline>({
      buildSearchParams: (baseline) => baseline,
      ...
      initialProps: {isBaselineReady: false, search: {}},
    })

    await waitFor(() => expect(navigate).toHaveBeenCalledTimes(1))
    expect(navigate.mock.calls[0]?.[0].search()).toEqual({person: 'user-2'})

    rerender({isBaselineReady: true, search: {person: 'user-2'}})
  ...
  it('preserves explicit search until the baseline is ready, then merges personal and shared state once', async () => {
    const navigate = vi.fn()
    const explicitSearch = {sort: 'priority:desc'}

    const {rerender} = renderHook(({isBaselineReady}: {isBaselineReady: boolean}) => useInitializeViewSearch<SearchParams, SharedBaseline>({
      buildSearchParams: (baseline) => baseline,
      getPersonalConfig: () => ({person: 'user-2'}),
      isBaselineReady,
      mergePersonalWithShared: (personal, shared) => ({
        ...shared,
        ...personal,
        ...explicitSearch,
      }),
      ...
      sharedBaseline: {person: 'shared-user'},
    }), {
      initialProps: {isBaselineReady: false},
    })
  ...
  it('merges explicit search with the shared baseline when no personal config exists', async () => {
    const navigate = vi.fn()
    const explicitSearch = {sort: 'priority:desc'}

    const {rerender} = renderHook(({isBaselineReady}: {isBaselineReady: boolean}) => useInitializeViewSearch<SearchParams, SharedBaseline>({
      buildSearchParams: (baseline) => baseline,
      getPersonalConfig: () => null,
      isBaselineReady,
      mergePersonalWithShared: (personal, shared) => ({
        ...shared,
        ...personal,
  ...
  it('stays uninitialized until the final merged search lands', async () => {
    const navigate = vi.fn()

    const {result, rerender} = renderHook(({search}: {search: SearchParams}) => useInitializeViewSearch<SearchParams, SharedBaseline>({
      buildSearchParams: (baseline) => baseline,
      getPersonalConfig: () => ({person: 'user-2'}),
      isBaselineReady: true,
      mergePersonalWithShared: (personal, shared) => ({...shared, ...personal}),
  ...
  it('marks initialization complete without navigating when the current search already matches the shared baseline', async () => {
    const navigate = vi.fn()
    const {result} = renderHook(() => useInitializeViewSearch<SearchParams, SharedBaseline>({
      buildSearchParams: (baseline) => baseline,
      isBaselineReady: true,
      navigate,
      routeKey: 'project-1:view-1',
