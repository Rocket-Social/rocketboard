...
describe('LazySurfaceBoundary', () => {
  ...
  it('lets users close a dialog lazy-load error instead of staying trapped behind the overlay', async () => {
    const user = userEvent.setup()
    const onDismiss = vi.fn()
    ...
    expect(screen.getByText('Complete sprint failed to load.')).toBeInTheDocument()
    await user.click(screen.getByRole('button', {name: 'Close'}))
    expect(onDismiss).toHaveBeenCalledTimes(1)
  })
})
