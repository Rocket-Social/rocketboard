import {ArrowLeft, Copy, Mail, MoreHorizontal, Search, Trash2, X} from 'lucide-react'
...
import {getEffectivePlan} from '../billing/entitlement.types'
...
export function OrgSettingsPage() {
  ...
  const search = useSearch({strict: false}) as {tab?: OrgSettingsTab}
  ...
  const setDomainsMutation = useSetAllowedDomainsMutation(orgId ?? '')
  ...
  const requestedTab: OrgSettingsTab = (search as {tab?: OrgSettingsTab}).tab ?? 'overview'
  const activeTab: OrgSettingsTab = SELF_HOSTED_HIDDEN_TABS.includes(requestedTab) && IS_SELF_HOSTED
    ? 'overview'
    : requestedTab
  ...
  const handleInvite = () => {
    ...
    if (!org || !email || !isEmailFormat(email)) return
    createInviteMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          setInviteEmail('')
          toast({title: IS_SELF_HOSTED ? `Invite created for ${email}` : `Invite emailed to ${email}`})
        },
        onError: (error) => {
          toast({
  ...
  const handleAddDomain = () => {
    ...
    if (!d || !org) return
    const updated = [...(org.allowedDomains ?? []), d]
    setDomainsMutation.mutate(updated, {
      onSuccess: () => {
        setNewDomain('')
        toast({title: `Domain ${d} added`})
      },
    })
  }
  ...
  const handleRemoveDomain = (domain: string) => {
    if (!org) return
    const updated = (org.allowedDomains ?? []).filter((d) => d !== domain)
    setDomainsMutation.mutate(updated)
  }

  const setTab = (tab: OrgSettingsTab) => {
    void navigate({
      search: {tab},
      to: '.',
    })
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-3'>
        <div className='rounded-xl border border-border-subtle bg-surface-base p-4'>
          ...
          <p className='mt-1 font-display text-2xl font-semibold text-text-strong'>{memberCount}</p>
        </div>
        <div className='rounded-xl border border-border-subtle bg-surface-base p-4'>
        ...
        {canManage ? (
          <div className='space-y-4'>
            <div className='rounded-2xl border border-border-subtle bg-surface-base p-4'>
              <h3 className='text-xs font-semibold uppercase tracking-wider text-text-muted'>Invite by email</h3>
              <div className='mt-3 flex flex-wrap gap-2'>
                ...
                <Button disabled={!isEmailFormat(inviteEmail) || createInviteMutation.isPending} onClick={handleInvite} variant='primary'>
                  Add
                </Button>
              </div>
            </div>
            ...
            <div className='grid gap-4 xl:grid-cols-2'>
              ...
              <div className='rounded-2xl border border-border-subtle bg-surface-base p-4'>
                <h3 className='text-xs font-semibold uppercase tracking-wider text-text-muted'>Allowed domains</h3>
                <div className='mt-3 flex flex-wrap gap-2'>
                  {(org.allowedDomains ?? []).length > 0 ? (
                    org.allowedDomains.map((domain) => (
                      <span className='inline-flex items-center gap-1 rounded-full bg-surface-muted px-3 py-1 text-sm text-text-strong' key={domain}>
                        {domain}
                        <button
                    ...
                    <p className='text-sm text-text-muted'>No email domains are auto-approved yet.</p>
                  )}
                </div>
                <div className='mt-3 flex flex-wrap gap-2'>
                  <Input
                    ...
                    value={newDomain}
                  />
                  <Button disabled={!newDomain.trim()} onClick={handleAddDomain} size='compact' variant='secondary'>
                    Add
                  </Button>
                </div>
              </div>
