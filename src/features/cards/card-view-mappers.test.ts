...
import {buildBoardTasks, buildGanttTasks, taskBoardBacklogId, buildTableGroups} from './card-view-mappers'
...
describe('buildTableGroups (sprint)', () => {
  ...
  it('backlog appears after planned, before completed', () => {
    ...
    expect(ids.indexOf('__backlog')).toBeLessThan(ids.indexOf('sprint-completed'))
  })

  it('completed sprints appear last', () => {
    ...
    expect(ids[ids.length - 1]).toBe('sprint-completed')
  })
