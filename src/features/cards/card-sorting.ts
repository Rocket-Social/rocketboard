...
function buildPrioritySortRank(priorityOptions: ProjectPriorityOption[]): Map<string | null, number> {
  const rankMap = new Map<string | null, number>()
  rankMap.set(null, 9999) // null = no priority sorts LAST
...
function compareByField(
  ...
  switch (fieldKey) {
    ...
    case 'priority': {
      ...
      return compareRankValues(priorityRank.get(left.priorityOptionId) ?? 9999, priorityRank.get(right.priorityOptionId) ?? 9999, direction)
    }
    case 'start_date':
...
export function sortCards(
  ...
  return [...cards].sort((left, right) => {
    ...
    if (groupBy === 'assignee' || groupBy === 'due_date' || groupBy === 'priority') {
