...
const testPriorityOptions = [
  {id: 'prio-urgent', key: 'urgent', label: 'Urgent', sortOrder: 0, isDefault: false, color: 'red'},
  {id: 'prio-high', key: 'high', label: 'High', sortOrder: 1, isDefault: false, color: 'amber'},
  {id: 'prio-medium', key: 'medium', label: 'Medium', sortOrder: 2, isDefault: true, color: 'blue'},
]
...
describe('sortCards', () => {
  ...
  it('sorts by priority option rank', () => {
    const cards = [
      makeCard({priorityOptionId: 'prio-medium'}),
      makeCard({priorityOptionId: 'prio-urgent'}),
      makeCard({priorityOptionId: 'prio-high'}),
    ]
    const sorted = sortCards(cards, [{fieldKey: 'priority', direction: 'asc'}], [], {priorityOptions: testPriorityOptions})
    expect(sorted.map((c) => c.priorityOptionId)).toEqual(['prio-urgent', 'prio-high', 'prio-medium'])
  })

  it('keeps missing priorities last when sorting by priority desc', () => {
    const cards = [
      makeCard({id: 'urgent', priorityOptionId: 'prio-urgent'}),
      makeCard({id: 'none', priorityOptionId: null}),
      makeCard({id: 'high', priorityOptionId: 'prio-high'}),
    ]
    const sorted = sortCards(cards, [{fieldKey: 'priority', direction: 'desc'}], [], {priorityOptions: testPriorityOptions})
    expect(sorted.map((c) => c.id)).toEqual(['high', 'urgent', 'none'])
  })
  ...
  it('can force createdAt as the tiebreaker even when the view is grouped by group', () => {
    const cards = [
      makeCard({createdAt: '2026-03-25T10:10:00.000Z', groupPosition: 0, id: 'late', title: 'Same'}),
      makeCard({createdAt: '2026-03-25T10:00:00.000Z', groupPosition: 1, id: 'early', title: 'Same'}),
    ]
    ...
    expect(sorted.map((card) => card.id)).toEqual(['early', 'late'])
  })
