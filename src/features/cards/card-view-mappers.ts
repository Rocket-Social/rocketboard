...
export const ASSIGNEE_UNASSIGNED_COLUMN_ID = '__unassigned'
...
type SprintPartitionOptions = {
  sprintOrder?: 'default' | 'input'
}

type BuildTableGroupsOptions = SprintPartitionOptions
...
// Wave 2 AI Kanban Phase 4 (PR 4-B-2): assignee groupBy for BoardView.
//
// Returns a `ProjectBoardLayout` whose columns are humans + agents +
// unassigned. Drop targets in this layout map to user IDs (or
// `ASSIGNEE_UNASSIGNED_COLUMN_ID`); the route turns a drop into a
// `setCardAssignee` mutation. Phase 2a's `cards_assignee_dispatch`
// trigger handles agent dispatch downstream when the new assignee
// resolves to a persona's `agent_user_id`. Cards whose assignee is
// neither a project member nor a known agent fall into the unassigned
// column so they aren't silently dropped.
export type AssigneeBoardColumnAccent = 'neutral' | 'agent'

export type AssigneeBoardColumnMeta = {
  accent: AssigneeBoardColumnAccent
...
export type AssigneeBoardLayout = ProjectBoardLayout & {
  columnMeta: AssigneeBoardColumnMeta[]
}

export function buildAssigneeBoardTasks(
  cards: CardRecord[],
  priorityOptions: ProjectPriorityOption[],
  ...
  const sortedPersonas = [...assignablePersonas].sort((left, right) =>
    left.name.localeCompare(right.name),
  )

  const columnMeta: AssigneeBoardColumnMeta[] = [
    ...sortedMembers.map<AssigneeBoardColumnMeta>((member) => ({
      accent: 'neutral',
      accentColor: null,
      id: member.id,
      isUnassigned: false,
    ...
    ...sortedPersonas.map<AssigneeBoardColumnMeta>((persona) => ({
      accent: 'agent',
      accentColor: persona.accentColor ?? null,
      id: persona.agentUserId,
      isUnassigned: false,
      title: persona.name,
    ...
    })),
    {
      accent: 'neutral',
      accentColor: null,
      id: ASSIGNEE_UNASSIGNED_COLUMN_ID,
      isUnassigned: true,
      title: 'Unassigned',
      userId: null,
    },
  ...
  const lanes: ProjectBoardLane[] = [{id: taskBoardStandardLaneId, sprint: null, title: 'All tasks'}]
  const grouped: Record<string, Record<string, ProjectBoardTask[]>> = {}
  for (const meta of columnMeta) {
    grouped[meta.id] = {[taskBoardStandardLaneId]: []}
  }

  for (const card of cards) {
    const columnId = card.assigneeUserId && grouped[card.assigneeUserId]
      ? card.assigneeUserId
      : ASSIGNEE_UNASSIGNED_COLUMN_ID

    grouped[columnId][taskBoardStandardLaneId].push({
      assignee: toInitials(card.assigneeName),
      card,
      columnId,
      dueIn: daysUntil(card.dueAt),
  ...
  return {columnMeta, lanes, tasksByColumn: grouped}
}
...
export function buildSprintPartitions<T>(
  ...
  projectSprints: ProjectSprintRecord[],
  options: SprintPartitionOptions = {},
): SprintPartition<T>[] {
  const grouped = new Map<string, T[]>()
  const backlogItems: T[] = []
  const sortedSprints = options.sprintOrder === 'input'
    ? [...projectSprints]
    : sortProjectSprints(projectSprints)
...
export function buildTableGroups(
  ...
  projectSprints: ProjectSprintRecord[] = [],
  taskMode: TaskBoardMode = 'standard',
  options: BuildTableGroupsOptions = {},
): ProjectTableGroup[] {
  ...
  if (taskMode === 'sprint') {
    return buildSprintPartitions(cards, (card) => card.sprintId, projectSprints, options).flatMap((partition) => {
      ...
      const childGroups = groupBy === 'group'
        ? [{
            createDefaults: {groupId: null, sprintId: partition.sprint?.id ?? null},
            expanded: true,
