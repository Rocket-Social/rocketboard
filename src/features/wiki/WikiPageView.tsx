...
import { useCreateDialogs } from "../shell/CreateDialogsContext";
...
type WikiVersionHistoryListProps = {
...
function WikiVersionHistoryList({
  currentVersion,
  isLoading,
  isRemovePending,
  isRestorePending,
  ...
  onViewVersion,
  versions,
}: WikiVersionHistoryListProps) {
  const [showAll, setShowAll] = useState(false);
  const visibleVersions = showAll ? versions : versions.slice(0, 3);

  if (isLoading) {
    return (
      <div className="rounded-xl border border-dashed border-border-subtle px-4 py-3 text-sm text-text-muted">
        Loading revisions...
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3">
            <div>
              <p className="text-sm font-medium text-text-strong">
                v{entry.revisionNumber}
              </p>
              <p className="text-xs text-text-muted">
                {(entry.authorName || "Unknown")} · {formatWikiPageDate(entry.createdAt)}
              </p>
            </div>
            ...
            {entry.version === currentVersion ? (
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    aria-label={`Version actions for v${entry.revisionNumber}`}
                    className="flex h-8 w-8 items-center justify-center rounded-md text-text-muted hover:bg-canvas-accent hover:text-text-strong"
                    type="button"
                  >
...
export function WikiPageView({
  ...
  const { openCommandPalette } = useCreateDialogs();
  ...
  const versionsQuery = useWikiVersionsQuery(page.id);
  const restoreVersionMutation = useRestoreWikiPageVersionMutation(organizationId);
  const deleteVersionMutation = useDeleteWikiPageVersionMutation(page.id);
  ...
  const [viewingVersion, setViewingVersion] =
    useState<WikiPageVersionContent | null>(null);
  ...
  const applyRestoredVersion = useCallback(
    (restoredPage: WikiPageVersionMutationResult) => {
      clearSaveTimer();
      clearSaveStatusTimer();
      latestContentRef.current = restoredPage.contentJson;
      lastSavedContentRef.current = stringifyRichTextDocument(
        restoredPage.contentJson,
      );
      pageVersionRef.current = restoredPage.pageVersion;
      saveInFlightRef.current = false;
      queuedSaveRef.current = false;
      setTitle(restoredPage.pageTitle);
      setContent(restoredPage.contentJson);
      setViewingVersion(null);
      setSaveStatus("saved");
  ...
  const handleViewVersion = useCallback(
    async (versionId: string) => {
      try {
        const versionContent = await wikiPageRepository.getVersionContent({
          pageId: page.id,
          versionId,
        });
        setViewingVersion(versionContent);
      } catch {
        toast({ title: "Couldn't load revision", variant: "error" });
      }
    },
    [page.id, toast],
  );
  ...
  const handleRestoreVersion = useCallback(
    (versionId: string) => {
      if (saveInFlightRef.current) {
        toast({
          title: "Wait for the current save to finish before restoring.",
          variant: "error",
        });
        return;
      }
      ...
      restoreVersionMutation.mutate(
        ...
        {
          onError: (error) => {
            const message = getErrorMessage(error, "Couldn't restore revision");
            toast({
              title:
                message === "WIKI_PAGE_CONFLICT"
                  ? "This page was updated elsewhere. Reload to see the latest."
                  : "Couldn't restore revision",
              variant: "error",
            });
          },
          onSuccess: (restoredPage) => {
            applyRestoredVersion(restoredPage);

            if (restoredPage.pageSlug !== page.slug) {
              const newPath = buildWikiPagePath(
                {
                  parentPageId: page.parentPageId,
                  slug: restoredPage.pageSlug,
                },
                allPages,
              );
              onNavigateToSlugPath(newPath);
  ...
  const handleRemoveVersion = useCallback(
    (versionId: string) => {
      deleteVersionMutation.mutate(
        ...
        {
          onError: () =>
            toast({ title: "Couldn't remove revision", variant: "error" }),
        },
      );
    },
    [deleteVersionMutation, page.id, toast],
  );
  ...
  const aiSurfaceContext = useMemo(
    () =>
      aiChatOpen
        ...
        : undefined,
    [aiChatOpen, allPages, content, page, title],
  );
    ...
    <div className="flex h-full flex-col overflow-y-auto">
      ...
      <div className="flex items-center gap-1.5 border-b border-border-subtle px-6 py-3 text-sm text-text-muted">
        ...
        <div className="ml-auto">
          <HeaderSearchIconButton
            onOpen={() => void openCommandPalette()}
            variant="quiet"
          />
        </div>
      </div>
      ...
      <div className="flex-1 px-6 py-6 lg:px-12">
        ...
        <input
          ...
          disabled={Boolean(viewingVersion)}
          onBlur={viewingVersion ? undefined : handleTitleBlur}
          onChange={(e) => setTitle(e.target.value)}
          onKeyDown={viewingVersion ? undefined : handleTitleKeyDown}
          placeholder={showNewPageTitlePlaceholder ? "New Page Title" : "Untitled"}
          ref={titleInputRef}
          value={viewingVersion?.title ?? title}
        />
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3 text-sm">
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            >
              ...
              {isDesktop ? <span className="ml-1 text-xs">AI</span> : null}
            </Button>
            <Button
        ...
        {viewingVersion ? (
          <div className="mb-5 flex items-center gap-3 rounded-xl border border-primary/20 bg-primary-soft px-4 py-3 text-sm text-primary">
            <Eye className="h-4 w-4" />
            <span className="flex-1">
              Viewing <strong>v{viewingVersion.revisionNumber}</strong> by{" "}
              {viewingVersion.authorName || "Unknown"}
            </span>
            <Button
        ...
        >
          <LazyRichTextEditor
            ...
            value={viewingVersion?.contentJson ?? content}
          />
        </Suspense>
        ...
        <div className="mt-8 border-t border-border-subtle pt-6">
          ...
          <WikiVersionHistoryList
            currentVersion={pageVersionRef.current}
            isLoading={versionsQuery.isLoading}
            isRemovePending={deleteVersionMutation.isPending}
      ...
      <AiChatDrawer
        ...
        suggestedPrompts={WIKI_PAGE_PROMPTS}
        surface="wiki"
