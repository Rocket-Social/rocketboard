...
export function useRestoreWikiPageVersionMutation(orgId: string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (input: RestoreWikiPageVersionInput) => wikiPageRepository.restoreVersion(input),
    onSuccess: () => {
      void queryClient.invalidateQueries({queryKey: wikiKeys.all})
      void queryClient.invalidateQueries({queryKey: wikiKeys.orgPages(orgId)})
    },
...
export function useDeleteWikiPageVersionMutation(pageId: string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (input: DeleteWikiPageVersionInput) => wikiPageRepository.deleteVersion(input),
    onSuccess: (_result, input) => {
      void queryClient.invalidateQueries({queryKey: wikiKeys.versions(input.pageId || pageId)})
    },
  })
}
