...
import {Copy, Globe, Link2, RotateCcw} from 'lucide-react'
...
import {
  wikiShareQueryOptions,
  useCreateWikiShareLinkMutation,
  useRevokeWikiShareLinkMutation,
} from './wiki.queries'

type WikiShareDialogProps = {
  onClose: () => void
  pageId: string
}

export function WikiShareDialog({onClose, pageId}: WikiShareDialogProps) {
  const shareQuery = useQuery(wikiShareQueryOptions(pageId))
  const createShareMutation = useCreateWikiShareLinkMutation(pageId)
  const revokeShareMutation = useRevokeWikiShareLinkMutation(pageId)
  const [copied, setCopied] = useState(false)
  const {toast} = useToast()

  const shareUrl = useMemo(() => {
    if (!shareQuery.data?.shareToken || typeof window === 'undefined') return ''
    return `${window.location.origin}/shared/wiki/${shareQuery.data.shareToken}`
    ...
    <Dialog open onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className='w-[min(32rem,calc(100vw-2rem))] rounded-[28px] bg-surface-base'>
        <DialogHeader className='px-6 py-5'>
          <p className='font-mono text-xs uppercase tracking-[0.24em] text-text-muted'>Share Wiki Page</p>
          <DialogTitle className='mt-1 font-display text-xl'>Publish a read-only page.</DialogTitle>
          <DialogDescription className='mt-2'>Anyone with the link can view this page without signing in.</DialogDescription>
        </DialogHeader>
        ...
        <div className='space-y-4 px-6 py-5'>
          <div className='rounded-3xl border border-border-subtle bg-surface-elevated p-4'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <p className='text-sm font-medium text-text-strong'>Public wiki page</p>
