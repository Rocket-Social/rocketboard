...
export const wikiPageRepository = {
  ...
  async listVersions(pageId: string): Promise<WikiPageVersionRecord[]> {
    const {data, error} = await getSupabaseBrowserClient()
      ...
      .select('id, version, title, created_by_user_id, created_at')
      .eq('page_id', pageId)
      .order('version', {ascending: false})

    if (error) throw error

    type VersionRow = {id: string; version: number; title: string; created_by_user_id: string; created_at: string}
