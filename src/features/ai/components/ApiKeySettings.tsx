...
export function ApiKeySettings({ organizationId }: ApiKeySettingsProps) {
  ...
  const membersQuery = useOrgMembersQuery(organizationId)
  const [savedCredentialKey, setSavedCredentialKey] = useState<string | null>(null)
  const [savedOrgCredentialKey, setSavedOrgCredentialKey] = useState<string | null>(null)

  const userKeys = statusQuery.data?.userKeys ?? []
  const orgKeys = statusQuery.data?.orgKeys ?? []
  ...
  const handleSaveOrg = (
    ...
    setKeyMutation.mutate(
      { credentialKind, key, provider, scope: 'org' },
      {
        onError: (error) => {
          toast({
            description: getErrorMessage(error, 'Could not save the organization API key.'),
            title: 'Save failed',
            variant: 'error',
          })
        },
        onSuccess: () => {
          const credentialKey = `${provider}:${credentialKind}`
          setSavedOrgCredentialKey(credentialKey)
          setTimeout(() => setSavedOrgCredentialKey(null), 2000)
          toast({
            description: `Organization ${AI_PROVIDER_LABELS[provider]} key saved. AI agents in this org now use it.`,
            title: 'Organization API key saved',
          })
        },
      },
    )
  ...
  const handleClearOrg = async (
    ...
    const confirmed = await confirm({
      confirmLabel: 'Delete key',
      title: `Delete the organization ${credentialLabel}?`,
      variant: 'destructive',
    })
    ...
    clearKeyMutation.mutate(
      { credentialKind, provider, scope: 'org' },
      {
        onError: (error) => {
          toast({
            description: getErrorMessage(error, 'Could not delete the organization API key.'),
            title: 'Delete failed',
            variant: 'error',
          })
        },
        onSuccess: () => {
          toast({
            description: `Organization ${credentialLabel} cleared. Members fall back to personal keys.`,
            title: 'Organization API key deleted',
          })
        },
      },
    )
  ...
  return (
    <>
      {canManage ? (
        <section className='mb-6 rounded-3xl border border-border-subtle bg-surface-elevated p-5 shadow-panel'>
          ...
          <p className='mt-2 text-sm text-text-muted'>
            Add a key once for the whole org. It applies to all AI agent runs in
            this organization. Charges go to the org admin&apos;s provider account.
          </p>
          ...
          ) : statusQuery.isError ? null : (
            <div className='mt-4 space-y-3'>
              <ApiKeyProviderCard
                existingKey={orgKeys.find((key) => key.provider === 'anthropic' && key.credentialKind === 'api_key') ?? null}
                isSaving={setKeyMutation.isPending}
                onClear={() => handleClearOrg('anthropic', 'api_key')}
                onSave={(key) => handleSaveOrg('anthropic', key, 'api_key')}
                provider='anthropic'
                savedProvider={savedOrgCredentialKey === 'anthropic:api_key' ? 'anthropic' : null}
              />

              {PROVIDERS.filter((provider) => provider !== 'anthropic').map((provider) => {
                const existing = orgKeys.find((key) => key.provider === provider && key.credentialKind === 'api_key') ?? null
                return (
                  <ApiKeyProviderCard
                    existingKey={existing}
                    isSaving={setKeyMutation.isPending}
                    key={`org-${provider}`}
                    onClear={() => handleClearOrg(provider, 'api_key')}
                    onSave={(key) => handleSaveOrg(provider, key, 'api_key')}
                    provider={provider}
                    savedProvider={savedOrgCredentialKey === `${provider}:api_key` ? provider : null}
                  />
                )
              })}
            </div>
