...
export type PersonaFormValues = {
  ...
  // Phase 7-C: rolling-60-min rate cap. NULL = admin opt-out (no cap).
...
export function PersonaEditDialog({
  ...
  // Phase 7-C: keep the rate cap as a string input so the empty-state
  // (admin opt-out → null) round-trips cleanly. The number coercion
  // happens in `normalizedValues`.
  const [maxRunsPerHour, setMaxRunsPerHour] = useState<string>(
    persona.maxRunsPerHour === null ? '' : String(persona.maxRunsPerHour ?? 60),
  )
  ...
  useEffect(() => {
    ...
    setMaxRunsPerHour(persona.maxRunsPerHour === null ? '' : String(persona.maxRunsPerHour ?? 60))
  }, [isOpen, persona])
  ...
  const normalizedValues = useMemo<PersonaFormValues>(() => {
    ...
    // Empty input → null (admin opt-out, T7-5). Otherwise parse to a positive
    // integer in [1, 9999]; out-of-range or non-numeric collapses to null so
    // the canSave gate refuses to submit.
    const trimmedRate = maxRunsPerHour.trim()
    let normalizedRate: number | null = null
    if (trimmedRate !== '') {
      const parsed = Number(trimmedRate)
      normalizedRate = Number.isFinite(parsed) && parsed >= 1 && parsed <= 9999
        ? Math.floor(parsed)
        : null
    }
    ...
    return {
      ...
      maxRunsPerHour: trimmedRate === '' ? null : normalizedRate,
      model: model.trim(),
      name: name.trim(),
  ...
  const initialValues = useMemo<PersonaFormValues>(() => ({
    ...
    maxRunsPerHour: persona.maxRunsPerHour,
    model: persona.model,
    name: persona.name,
  ...
  // Reject submit when the rate-cap input is non-empty but doesn't parse
  // into the [1, 9999] range. Empty (= admin opt-out) is allowed.
  const rateInputTrimmed = maxRunsPerHour.trim()
  const rateInputIsValid = rateInputTrimmed === '' || normalizedValues.maxRunsPerHour !== null
      ...
      <DialogContent className='h-[min(46rem,calc(100vh-2rem))] w-[min(42rem,calc(100vw-2rem))] overflow-y-auto rounded-[28px] bg-surface-base p-0'>
        ...
        <div className='space-y-6 px-6 py-5'>
          ...
          <section className='rounded-3xl border border-border-subtle bg-surface-elevated p-5 shadow-panel'>
            ...
            <div className='mt-4'>
              <label className='space-y-2'>
                <span className='text-sm font-medium text-text-strong'>Max runs per hour</span>
                <Input
              ...
              <p className='mt-2 text-xs text-text-muted'>
                Caps how often this agent gets picked up by the worker (rolling 60-min window).
                Leave blank to remove the cap. Default is 60 runs/hour.
              </p>
              {!rateInputIsValid ? (
                <p className='mt-2 text-xs text-rose-700'>
                  Enter a whole number between 1 and 9999, or leave blank to remove the cap.
                </p>
              ) : null}
            </div>
          </section>
